...
- 부모-학생 연결은 선택 기능이며 가입 또는 학원 학생 등록의 필수 조건이 아님
- 부모는 학원 멤버가 아니며 학생과 연결된 경우에만 자녀 정보를 볼 수 있음
- 부모-학생 연결은 초대코드 직접 입력 방식이 아니라 양방향 초대장 수락/거절 방식으로 처리함
```
...
학생 관리 UX:
- 학원이 학생 정보를 입력해 비회원 프로필을 생성합니다.
- 기존 학생 계정이 있는 경우 초대장을 보내 수락 후 연결합니다.
- 학생이 아직 Ringdu 계정이 없는 경우에도 등록 가능합니다. (StudentProfile)
- 어린 학생처럼 본인 연락처가 없는 경우 보호자 전화번호만 입력하여 관리할 수 있습니다.
- 부모-학생 연결은 선택이며, 보호자 전화번호는 학원에서 연락하기 위한 용도로만 저장됩니다.
- 학생 상세 화면에서 기본 정보와 보호자 연락처를 우선 제공합니다.
...
`/student`는 보호자 연결 초대 보내기, 받은 초대장, 보낸 초대장, 연결된 보호자를 표시한다. 보호자 연결이 없어도 학생 대시보드는 정상 표시된다.
...
`/parent`는 자녀 연결 초대 보내기, 받은 초대장, 보낸 초대장, 연결된 자녀를 표시한다. 자녀 연결은 선택 기능이며, 연결 전에도 학부모 대시보드는 정상 표시된다. 부모는 학원 멤버가 아니고, 연결된 학생 범위의 정보만 확인한다.
